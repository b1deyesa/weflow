<x-layout.admin>
    ...
    <x-form action="{{ route('admin.team.store') }}" method="POST" enctype="multipart/form-data" class="mt-4">
        <div class="d-flex gap-4">
            <div class="d-flex flex-column w-100">
                <x-input label="Team Name" type="text" name="name" />
                <x-input label="Coach" type="select-search" name="employee_id" :options="$employees" />
            </div>
            ...
            <x-slot:body>
                @foreach ($customers as $customer)
                    <tr>
                        ...
                        <td width="1%">
                            <input type="checkbox" class="btn-check" name="customer_id[{{ $customer->id }}]" id="customer-{{ $customer->id }}" value="{{ old('customer_id[' . $customer->id . ']') }}" autocomplete="off">
